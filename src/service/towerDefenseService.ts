  ...
  async recordGameSession(gameData: {
    ...
    try {
      ...
      if (error) {
        ...
        return { code: -1, data: { message: '记录游戏会话失败' } }
      }

      return { code: 200, data: data[0] }
    } catch (error: any) {
      console.error('记录游戏会话异常:', error)
      return { code: -1, data: { message: '记录游戏会话异常: ' + error.message } }
    }
  },

  // 获取用户关卡进度
  async getUserLevelProgress(userId: string, levelId?: string) {
    try {
      let query = supabase
      ...
      console.error('获取用户关卡进度异常:', error)
      return { code: -1, data: [] }
    }
  },
